...
{
    public abstract class webWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static webWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(webWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<webDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<webDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<webDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<webDbContext, Task<T>> func)
        {
            T result;
